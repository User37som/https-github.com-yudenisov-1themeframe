<?php
function tf_selectors_new($template) {

	global $thfr_css;

	$thisSelectorgroup = $template; 
	...
	if(isset($thisSelectorgroup)) { 
		...
		if(in_array($thisSelectorgroup, $all_page_templates )) {
			...
			else $selector_prefix = ''; // Avoid "div.index"
			
			foreach(array('layout', 'breadcrumbs', 'searchform', 'multinav', 'singlenav', 'post', 'widget', 'hormenu', 'hormenu2', 
				'widgetarea_1', 'widgetarea_1_2', 'widgetarea_1_3', 'widgetarea_2', 'widgetarea_2_2', 'widgetarea_3', 
				'widgetarea_4', 'widgetarea_5', 'widgetarea_6', ) as $layoutsection) {
