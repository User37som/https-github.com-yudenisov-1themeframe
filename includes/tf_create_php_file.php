<?php function tf_create_php_file($thisTemplate) {
global $thfr_css;
$sitetitle_item = '
...
if (is_array($thfr_css['customdropimages']) AND !empty($thfr_css['customdropimages'])) {
	foreach ( $thfr_css['customdropimages'] as $img ) {
		$string = '';
		if ($img['link'] == 'on' ) {
			if ($img['linktype' == 'custom'] AND $img['customurl'] != '') $url = $img['customurl'];
			else $url = '<?php echo home_url( "/" ); ?>';
			if ($img['linktitle'] != '') $title = ' title="' . $img['linktitle'] . '"';
			else $title = '';
			if ($img['linktarget'] != '') $target = ' target="' . $img['linktarget'] . '"';
			else $target = '';	
			if ($img['linkrel'] != '') $rel = ' rel="' . $img['linkrel'] . '"';
			else $rel = '';	
			
			$linkid = str_replace('.', '', $img['file']) . '-link';
			$string .= '<a id="' . $linkid . '" href="' . $url . '"' . $title . $target . $rel . '>';
		}
		
		if($img['alt'] == 'on') $alt = ' alt="' . $img['alttext'] . '"';
		else $alt = '';
		$string .= '<img id="' . str_replace('.', '', $img['file']) . '" src="<?php bloginfo("template_url"); ?>/images/' . $img['file'] . '"' . $alt . ' />';
		
		if ($img['link'] == 'on' ) {
			$string .= '</a>';
		}
		
		$dropitemname = str_replace('.', '', $img['file']) . '_custimg';
		$$dropitemname = $string;
	}
}
// End cust img
...
if (is_array($thfr_css['customdroplinks']) AND !empty($thfr_css['customdroplinks'])) {
	foreach ( $thfr_css['customdroplinks'] as $linkid => $linkinfo ) {
		$string = '<a id="' . $linkid . '" href="' . $linkinfo['linkurl'] . '"';
		if($linkinfo['linktitle'] != '') $string .= ' title="' . $linkinfo['linktitle'] . '"';
		if($linkinfo['linktarget'] != '') $string .= ' target="' . $linkinfo['linktarget'] . '"';
		if($linkinfo['linkrel'] != '') $string .= ' rel="' . $linkinfo['linkrel'] . '"';
		$string .= '>' . $linkinfo['linktext'] . '</a>';
...
echo '<?php /* Template Name: ' . $thisTemplate . ' 
*/ ?>
...
echo tf_postcontainer_code($thisTemplate);
